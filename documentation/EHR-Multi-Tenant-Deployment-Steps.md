...
This guide walks through deploying the HIRTA EHR Multi-Tenant stack (backend Lambda functions, API Gateway, Cognito, DynamoDB, CloudFront + S3 dashboard website) from a clean machine.
...
2. Open the Ubuntu terminal and navigate to the project directory:
   ```bash
   cd /mnt/d/HIRTA/MOD-EHR/mod_ehr
   ```
...
4. Update the dashboard's API base URL to use `<domain>` (e.g. `dashboard.hirta.us`) instead of the CloudFront default domain.
